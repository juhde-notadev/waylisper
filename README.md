...
- Model: `small.en` via `WAYLISPER_MODEL` (set in `~/.config/systemd/user/transcribe-daemon.service.d/override.conf`, not the unit file itself). Started on `large-v3-turbo` but benchmarked both on this hardware: on natural conversational dictation the two produced byte-identical transcripts while `small.en` ran ~4.5x faster (4.24s vs 19.09s wall time including model load); a deliberately brutal jargon-heavy stress test showed both models struggling about equally on hard technical terms (GPG, sudoers, auth key), with `small.en` actually preserving proper nouns (`path0s`, `YubiKey`) slightly better than `base.en` did in the same test. `base.en` (10x faster than `large-v3-turbo`) was also benchmarked and rejected — it visibly mangled `path0s` and `YubiKey`, an unacceptable trade for the extra speed. CPU-only hardware (no dedicated GPU) makes model size the dominant latency lever once `beam_size=1`/`int8` are already applied, which they are by default in `bin/transcribe-daemon`.
- `transcribe-daemon` runs as a `systemctl --user` service (`~/.config/systemd/user/transcribe-daemon.service`, copied from `systemd/transcribe-daemon.service` in this repo — that unit file needed no changes either; the model override lives in a drop-in, not the unit file, so it stays out of version control's way if it ever needs tuning again).
